...
class MixtureFitState(NamedTuple):
    ...
    State of the mixture model fit.
    ...
    mu : Array
        Mean angles of the mixture components.
    kappa : Array
        Concentration parameters of the mixture components.
    logw : Array
        Logarithm of the mixture component weights.
    mask : Array
        Mask indicating valid data points.
    r : Array
        Responsibility matrix, indicating the probability of each data point
        belonging to each mixture component.
    log_likelihood : Array
        Log-likelihood of the mixture model at each iteration.
    n_iter : int
        Number of iterations completed.
    converged : bool
        Whether the mixture model has converged.
    atol : float
        Absolute tolerance for convergence.
    statuses : Array
        Status codes from the optimization routine at each iteration.
    njevs : Array
        Number of Jacobian evaluations at each iteration.
    nfevs : Array
        Number of function evaluations at each iteration.
    nits : Array
        Number of iterations taken by the optimization routine at each iteration.
    successes : Array
        Whether the optimization routine succeeded at each iteration.
    """
...
def em_step(
    ...
    state = state._replace(n_iter=state.n_iter + 1, converged=converged)
    return state
...
def load_mixture_state(f):
    ...
    return MixtureFitState(
        **{f: jnp.array(states_loaded.get(f)) for f in states_loaded.files}
    )
