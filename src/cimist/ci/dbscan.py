...
def compute_distances(mixture: MixtureFitState) -> Array:
    ...
    normalize_cross_counts = lambda x: jnp.sqrt(jnp.diag(1/jnp.diag(x)))@x@jnp.sqrt(jnp.diag(1/jnp.diag(x)))
...
def dbscan_resample_eps(D: Array, weights: Array, r: Array, eps: float, key: Array, min_probability_mass: float,
N: int=100):
...
def dbscan_trace_eps(D: Array, weights: Array, r: Array,
...
def dbscan_eps_std(D: Array, weights: Array, r: Array,
    k: ArrayLike=1,
    min_probability_mass: float=0.01) -> ResidueStates:
    """
    ...
    states, counts = jnp.unique(states_traj,
                size=D.shape[0], fill_value=-1, return_counts=True
                )
    ...
    return ResidueStates(n_states, S_mle, se, states_traj, counts, S_mle_dbs, dbs.w, eps, min_probability_mass, dbs.C)
