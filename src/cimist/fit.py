...
dynamics trajectory files.""",
    ...
    parser.add_argument(
        ...
        help="""Prior to use for residue entropy and pairwise mutual information
estimation with the Dirichlet distribution.
...
def time_and_message(message, func, printer=logging.info):
    """Decorator to time a function and log a message."""
    ...
    def wrapped(*args, **kwargs):
        ...
        printer(
            message + f" in {int(minutes)} minutes and {round(seconds, 3)} seconds."
        )
        return result
...
def combine_mixture_fit_batches(batches):
    """Combine mixture fit batches."""
    return ci.vmm.MixtureFitState(*[jnp.concatenate(x) for x in list(zip(*batches))])


def load_dataset(xyz, topology, stride=1) -> MDData:
    """Load molecular dynamics dataset."""
    traj = io.load_traj(xyz, topology, stride=stride)
...
def to_single_chain(traj):
    """Convert trajectory to a single chain."""
...
def main():
    ...
    # CONFIGURE LOGGING
    output_prefix = f"{input_args.output_prefix}_seed_{input_args.seed}" + os.sep
    if not os.path.exists(output_prefix):
        os.mkdir(output_prefix)

    logfile = output_prefix + "logging.txt"
    logging.basicConfig(filename=logfile, level=logging.INFO, filemode="w")
    logging.info("Began pipeline at %s .", today.strftime("%Y-%m-%d %H:%M:%S"))

    data = load_dataset(input_args.trajectory, input_args.topology)
    reference_structure = md.load(input_args.topology)
    ...
    init_keys = make_keys(input_args.seed)
    ...
    init_states_func = vmap(ci.vmm.init_random_mixture_state)
    ...
    mixture_state = combine_mixture_fit_batches(
        [
            init_states_func(*arrs)
            for arrs in zip(angles_batches, mask_batches, keys_batches)
        ]
    )
    ...
    step_if_converged = jit(
        partial(ci.vmm.step_if_not_converged, gtol=1e-3, gmaxiter=500)
    )
    ...
    for i in range(100):
        ...
        logging.info(
            "Reached convergence for %s out of %s residues.",
            jnp.sum(mixture_state.converged),
            len(mixture_state.converged),
        )

    if mixture_state.r.sum() == 0:
        new_r = vmap(ci.vmm.e_step)(
            data.angles,
            mixture_state.mu,
            mixture_state.kappa,
            mixture_state.logw,
    ...
    distances = ci.dbscan.compute_distances(mixture_state)
    weights = mixture_state.r.mean(axis=1)

    states = vmap(ci.dbscan.dbscan_eps_std)(distances, weights, mixture_state.r)
    tree = MIST.from_residue_states(
        states,
        [str(r) for r in data.residues],
        prior=input_args.prior,
        uncertainty=True,
    )
    ...
    io.save_tree_h5(tree, fname)
    pymol.tree_cartoon(tree, reference_structure, output_prefix + "pymol" + os.sep)
    io.save_coarse_graining_h5(states, fname)
    io.save_VMM_h5(mixture_state, fname)
    return None
