...
AA = set(mp_nerf.kb_proteins.SC_BUILD_INFO.keys()) - set("_")  # type: ignore


def aa_to_index(seq):
    """Converts a sequence of amino acids to an index array"""
    return np.vectorize({v: k for k, v in enumerate(AA)}.__getitem__)(list(seq))
...
def calculate_angle(c1, c2, c3):
    ...
    return jnp.arctan2(jnp.linalg.norm(jnp.cross(u1, u2)), jnp.dot(u1, u2))
...
def xyz_to_internal_coords(xyz):
    ...
    dih_quads = jnp.stack(
        [lax.dynamic_slice(xyz, (i, 0), (xyz.shape[0] - 3, 3)) for i in jnp.arange(4)],
        axis=-2,
    )
...
def get_data_masks(coords, point_ref):
    ...
    bond_mask = bond_mask.at[mask, 4].set(0.0)
    angles_mask = angles_mask.at[:, mask, 4].set(0.0)
