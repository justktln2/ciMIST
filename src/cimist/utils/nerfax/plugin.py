...
def protein_fold(
    ...
    for i in range(3, 14):
        ...
        coords = coords.at[level_mask, i].set(
            vmap(mp_nerf_jax)(
                coords_a,
...
def get_jax_protein_fold(scaffolds, only_backbone=False, reconstruct_fn=reconstruct_from_internal_coordinates):
    ...
    """
    cloud_mask, point_ref_mask = [
        jnp.array(x.cpu() if isinstance(x, torch.Tensor) else x)
        for x in map(scaffolds.__getitem__, ["cloud_mask", "point_ref_mask"])
    ]
