...
def chain_sequential_rotations_scan(rotations: Array) -> Array:
    ...
    _, rotations = jax.lax.scan(f=_body, init=jnp.eye(3), xs=rotations)
    return rotations
...
def work_inefficient_all_prefix_sum(operator, x):
    ...
    for j in range(0, j_max - 1):
        ...
        if r_max > n:
            ...
            l = jnp.concatenate(
                [
                    jax.lax.slice_in_dim(l, 0, 2**j),
                    jax.lax.slice_in_dim(r_, 0, n - 2 ** (j + 1)),
                ]
            )
    ...
    return jnp.concatenate(
        [
            jax.lax.slice_in_dim(prev_l, 0, 2**j),
            jax.lax.slice_in_dim(r_, 0, 2**j),
            final_r_,
        ]
    )
...
def reconstruct_from_internal_coordinates_pure_sequential(lengths, angles, dihedrals):
    ...
    def _body(ba, cb, c, length, sin_angle, cos_angle, sin_dihedral, cos_dihedral):
        d, dc = mp_nerf_jax_pretrig_self_normalising(
            ba, cb, c, length, sin_angle, cos_angle, sin_dihedral, cos_dihedral
        )
        return (cb, dc, d), (d,)
...
def reconstruct_from_internal_coordinates(
    ...
    if mode == "fully_sequential":
        ...
        return reconstruct_from_internal_coordinates_pure_sequential(
            *jax.tree_util.tree_map(lambda x: x.reshape(-1), (l, theta, chi))
        )

    ref_pos = jnp.array(
        [[-1, -1, 0], [-1, 0, 0], [0, 0, 0]], dtype=float
    )  # This choice makes the axis matrix identity so we can skip a matmul
