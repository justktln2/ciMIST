...
def reconstruct_from_internal_coordinates(l, theta, chi, mode='associative', device: Optional[Device]=None):
    ...
    if mode=='fully_sequential':
        ...
        return reconstruct_from_internal_coordinates_pure_sequential(*jax.tree_util.tree_map(lambda x: x.reshape(-1), (l, theta, chi)))
    ...
    split = lambda x, axis=-1: jax.tree_util.tree_map(lambda x: x.squeeze(axis), jnp.split(x, x.shape[axis], axis=axis))
